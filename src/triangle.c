/* Last modified Aug 28th 2026
/*****************************************************************************/
/*                                                                           */
/*                  Adapted to run on Windows x64 computers                  */
/*                          to be used with Ortho4XP                         */
/*                      for the X-Plane flight simulator                     */
/*                                                                           */
/*                         Modifications inpired from                        */
/*                      Traingle4XP.c  (v. Feb 21st 2024)                    */
/*                             by Oscar Pilote                               */
/*                                                                           */
/*                       triangle.exe file created with                      */
/*                 gcc (Rev3, Built by MSYS2 project) 16.2.0                 */
/*                                                                           */
/*                                                                           */
/*                              domisilasol, August 28th 2026                */
/*                                                                           */
               ...
               char *polyfilename, FILE **polyfile)
{
  ...
  for (i = 0; i < m->invertices; i++) {
    ...
    for (j = 0; j < m->nextras; j++) {
      stringptr = findfield(stringptr);
      if (*stringptr == '\0') {
        vertexloop[j+2] = 0.0;
      } else {
        vertexloop[j+2] = strtod(stringptr, &stringptr);
      }
    }
    if (nodemarkers) {
