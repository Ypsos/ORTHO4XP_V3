...
################################################################################
def export_airports_png(tile, dico_airports):
    ...
    png_file_name = os.path.join(
        tile.build_dir,
        FNAMES.short_latlon(tile.lat, tile.lon) + " Aéroports.png",
    )

    # --- LOCALISATION DES POLICES (MAC, WINDOWS, LINUX) ---
    def get_system_font(size, bold=False):
        ...
        if sys_os == "Darwin": # macOS
            ...
            paths = [os.path.join(root, "Fonts", "arialbd.ttf" if bold else "arial.ttf")]
        else: # Linux
            paths = ["/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf" if bold else "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",
                     "/usr/share/fonts/truetype/liberation/LiberationSans-Bold.ttf" if bold else "/usr/share/fonts/truetype/liberation/LiberationSans-Regular.ttf"]

        # 1. On tente les chemins physiques directs
        for p in paths:
            if os.path.exists(p):
                return ImageFont.truetype(p, size)

        # 2. On tente le chargement par nom (si l'OS a un cache de polices)
        try:
            return ImageFont.truetype("Arial Bold" if bold else "Arial", size)
        except:
            # 3. Message d'alerte et fallback final
            UI.vprint(1, f"   !! Police système recommandée absente. Affichage par défaut (taille réduite).")
            UI.vprint(1, f"   !! Conseil : Installez la police 'Arial' ou 'DejaVuSans' sur votre système.")
            return ImageFont.load_default()

    font_title = get_system_font(90, bold=True)
    font_header = get_system_font(56, bold=True)
    font_text = get_system_font(42)

    # --- CALCUL DYNAMIQUE DE LA LARGEUR ---
    title_raw = "AEROPORTS  " + FNAMES.short_latlon(tile.lat, tile.lon)
    title_spaced = " ".join(list(title_raw))

    tmp_draw = ImageDraw.Draw(Image.new("RGB", (1, 1)))
    try:
        # Mesure réelle pour que le titre (+50+003...) ne soit jamais coupé à droite
        bbox = tmp_draw.textbbox((0, 0), title_spaced, font=font_title)
        title_w = bbox[2] - bbox[0]
    except:
        title_w = 1200

    margin = 60
    col_w = [220, 320, 320, 1000]
    img_w = int(margin * 2 + max(sum(col_w), title_w))
    img_h = int(margin * 2 + 350 + (len(airport_list) * 80))
    ...
    # --- DESSIN DU TABLEAU ---
    y = margin
    draw.text((margin, y), title_spaced, fill=(30, 144, 255), font=font_title)

    y += 180
    headers = ["ICAO", "Latitude", "Longitude", "Nom"]
    curr_x = margin
    for i, h in enumerate(headers):
        draw.text((curr_x, y), h, fill=(60, 60, 60), font=font_header)
        curr_x += col_w[i]

    y += 90
    draw.line((margin, y, img_w - margin, y), fill="black", width=6)
    y += 70

    for apt in airport_list:
        lat = "{:.6f}".format(dico_airports[apt]["repr_node"][1])
        lon = "{:.6f}".format(dico_airports[apt]["repr_node"][0])
        name = dico_airports[apt]["name"]

        draw.text((margin, y), apt, fill="black", font=font_text)
        draw.text((margin + col_w[0], y), lat, fill="black", font=font_text)
        draw.text((margin + col_w[0] + col_w[1], y), lon, fill="black", font=font_text)
        draw.text((margin + col_w[0] + col_w[1] + col_w[2], y), name, fill="black", font=font_text)
        y += 80

    # SAUVEGARDE
    try:
        os.makedirs(os.path.dirname(png_file_name), exist_ok=True)
        img.save(png_file_name)
        UI.vprint(1, "   [OK] PNG d'information généré :", png_file_name)
    except Exception as e:
        UI.vprint(1, f"   !! Erreur lors de l'enregistrement du PNG : {e}")
